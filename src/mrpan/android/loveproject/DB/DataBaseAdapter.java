...
public class DataBaseAdapter {
	...
	public User getUser(String Name) {
		...
		if (cs != null) {
			while (cs.moveToNext()) {
				...
				user.setSex(cs.getInt(cs.getColumnIndex("user_sex")) == 1 ? true
						: false);
	...
	private Cursor UserCusor(String Name) {
		Cursor cs = mDb.query("User", null, "user_name='" + Name + "'", null,
				null, null, null);
		return cs;
	}
	...
	private Cursor DialogCursorByUser(String User, int Index) {
		return mDb.query("Dialog", null, "dia_user='" + User + "'", null, null,
				null, "dia_id limit 10 offset 10*" + (Index - 1));
	}

	private Cursor DialogCursorAll(int Index) {
		return mDb.query("Dialog", null, null, null, null, null,
				"dia_id limit 20 offset 20*" + (Index - 1));
	}
	...
	public Dialog getDialogByID(String ID) {
		Dialog d = null;
		Cursor cs = mDb.query("Dialog", null, "dia_id='" + ID + "'", null,
				null, null, null);
		if (cs != null) {
